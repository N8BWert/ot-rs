...
/// Platform Abstraction for the OpenThread Crypto Module
pub trait OTCryptoModule {
    type Error;

    /// Import a key into PSA ITS
    fn import_key(
        &mut self,
        key_ref: OTCryptoKeyRef,
        key_type: OTKeyType,
        key_algorithm: OTKeyAlgorithm,
        key_usage: u8,
        key_persistence: OTKeyStorage,
        key: &[u8],
    ) -> Result<(), Self::Error>;

    /// Export a key stored in PSA ITS, writing it into the buffer and returning
    /// the keys size.
    fn export_key(
        &mut self,
        key_ref: OTCryptoKeyRef,
    ...
    fn destroy_key(&mut self, key_ref: OTCryptoKeyRef) -> Result<(), Self::Error>;

    /// Check if the key ref passed has an associated key in PSA ITS.
    fn has_key(&mut self, key_ref: OTCryptoKeyRef) -> Result<bool, Self::Error>;

    /// Initialize the HMAC operation
    fn hmac_sha256_init(&mut self, context: u8) -> Result<(), Self::Error>;
}